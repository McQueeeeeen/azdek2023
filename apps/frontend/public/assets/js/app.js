(function () {
  ...
  function bindToastButtons() {
    document.addEventListener('click', function (e) {
      ...
      if (!btn) return;
      e.preventDefault();
      var msg = btn.getAttribute('data-toast') || 'Готово';
      showToast(msg, 'info');
    });
  }
    ...
    document.addEventListener('click', function (e) {
      ...
      if (addBtn) {
        ...
        item.innerHTML = '<div class="text-sm font-bold mb-2">' + label + '</div>' +
          '<div class="text-xs text-on-surface-variant">Новый адрес</div>' +
          '<div class="flex gap-4 mt-5 text-xs font-bold uppercase tracking-wider">' +
          '<button data-address-edit class="text-primary">Изменить</button>' +
          '<button data-address-delete class="text-slate-400">Удалить</button>' +
          '</div>';
  ...
  function bindAdminActions() {
    document.addEventListener('click', function (e) {
      ...
      if (!btn) return;
      e.preventDefault();
      var label = btn.getAttribute('data-admin-action') || 'Действие';
      showToast(label + ' в работе', 'info');
    });
  }
